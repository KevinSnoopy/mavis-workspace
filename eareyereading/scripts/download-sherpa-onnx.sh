...
echo "==> 下载 $TARBALL_URL"
curl -sL -o "$TMP_DIR/sherpa.tar.bz2" "$TARBALL_URL"
mkdir -p "$TMP_DIR/extracted"
tar xjf "$TMP_DIR/sherpa.tar.bz2" -C "$TMP_DIR/extracted"

echo "==> 放置 .so 到 $JNI_DIR"
rm -rf "$JNI_DIR"
mkdir -p "$JNI_DIR"
for abi in "${ABIS[@]}"; do
  ...
  mkdir -p "$JNI_DIR/$abi"
  cp "$src"/libsherpa-onnx-jni.so "$JNI_DIR/$abi/"
  cp "$src"/libonnxruntime.so     "$JNI_DIR/$abi/"
  echo "  $abi: $(ls -1 "$JNI_DIR/$abi" | tr '\n' ' ')"
done

echo "==> 拷贝 Kotlin 源码 Tts.kt 到 $SRC_DIR"
mkdir -p "$SRC_DIR"
...
curl -sL -o "$SRC_DIR/Tts.kt" "$TTS_KT_URL"
