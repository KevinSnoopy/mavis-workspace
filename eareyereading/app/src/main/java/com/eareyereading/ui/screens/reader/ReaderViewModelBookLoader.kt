...
internal class ReaderViewModelBookLoader(
    ...
    fun loadBook(bookId: Long) {
        ...
        // 加载生词本（用于阅读高亮）
        vocabJob = vm.viewModelScope.launch {
            try {
        ...
        bookJob = vm.viewModelScope.launch {
            try {
                ...
                // 初始化 TTS
                if (!vm._uiState.value.ttsInitialized) {
                    val ok = try {
                    ...
                    vm._uiState.update { it.copy(ttsInitialized = ok) }
                ...
                // 加载书签
                bookmarksJob?.cancel()
                bookmarksJob = vm.viewModelScope.launch {
                    try {
                ...
                // 加载高亮
                highlightsJob?.cancel()
                highlightsJob = vm.viewModelScope.launch {
                    try {
