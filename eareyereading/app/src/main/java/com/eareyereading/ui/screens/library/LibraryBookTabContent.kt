...
internal fun LibraryBookTabContent(
    ...
    LazyColumn(
        ...
        if (uiState.isLoading) {
            // 骨架屏：与真实卡片同构的占位 + 微光扫过，
            // 替代居中转圈（感知加载速度更快）；网格模式下用网格骨架
            if (uiState.viewMode == LibraryViewMode.GRID) {
                items(2, key = { "grid_skeleton_$it" }) {
                    Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                        repeat(GRID_COLUMNS) {
                            BookGridSkeleton(modifier = Modifier.weight(1f))
                        }
                    }
                }
            } else {
...
/**
 * 列表 ⇄ 封面网格切换（chips 行右端固定，不随横滑移出）。
 * 图标语义 = "点击后去往的视图"：当前列表时显示网格图标，反之亦然。
 */
@Composable
private fun ViewModeToggleButton(
    ...
    IconButton(onClick = {
        ...
        Icon(
            if (viewMode == LibraryViewMode.GRID) Icons.Default.ViewList else Icons.Default.GridView,
            contentDescription = if (viewMode == LibraryViewMode.GRID) "切换为列表" else "切换为网格",
