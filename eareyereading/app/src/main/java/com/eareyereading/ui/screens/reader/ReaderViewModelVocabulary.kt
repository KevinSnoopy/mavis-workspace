...
/**
 * 词汇域：点词查询（串行化、预合成）、加入生词本（去重、入复习队列）、
 * 弹窗关闭。
 */
internal class ReaderViewModelVocabulary(
    ...
    fun selectWord(word: String) {
        ...
        selectWordJob = vm.viewModelScope.launch {
            ...
            try {
                ...
                val sourceLang = vm._uiState.value.book?.language?.takeIf { it.isNotBlank() } ?: "en"
                // 如果没有释义，用 ML Kit 翻译
    ...
    fun addToVocabulary(word: String, context: String?) {
        ...
        val myBookId = vm.currentBookId
        vm.viewModelScope.launch {
            val currentVocab = vm._uiState.value.selectedVocab ?: return@launch
            // issue: 生词入库时把查好的释义一起持久化，否则"词汇本"里每词无翻译
            val wordDef = vm._uiState.value.wordDefinition
                ?.takeIf { it.isNotBlank() && it != "未找到释义" }
            val vocabToSave = currentVocab.copy(
                bookId = myBookId,
                bookTitle = vm._uiState.value.book?.takeIf { it.id == myBookId }?.title,
                context = context,
            ...
            try {
                ...
                if (existing != null) {
                    ...
                    // 到底加没加进去；补一条明确提示
                    vm._uiState.update { it.copy(showWordDialog = false, selectedVocab = null) }
                    vm.showToast("「$dedupeWord」已在生词本中")
                    return@launch
                }
                ...
                if (vm.currentBookId != myBookId) return@launch
