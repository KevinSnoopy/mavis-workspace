...
编译无错误**。另：发布后用户报告内置引擎英文书读成中文音（1×P1），
根因是默认模型为中文说话人，已按书籍语言路由语音模型修复，见第 5 节附录。
...
## 5. 附录：发布后修复 —— 内置引擎英文书读成中文音（P1）

**用户报告**：内置语音朗读不带情感语调，英文书里的数字被读出中文音。

**根因**（官方文档确认）：默认的 `vits-melo-tts-zh_en` 是从
**MeloTTS-Chinese 导出的单说话人模型——只有 1 个中文说话人**。
用它读英文：中文口音、语调平（"不带情感和语调"），英文数字词
被读出中文音（"数字用中文读出来"）。代码侧 `sid=0`、数字已转
英文词，均无法改变说话人音色这一根因。模型列表里虽有纯英文的
`vits-ljs`，但无选择入口、默认永远是中文模型。

**修复**（按书籍语言路由语音模型）：

- `EmbeddedTtsEngine`：
  - `resolveModelForLanguage(language)`：英文书 → 纯英文模型（理想目标，
    供引导/下载推荐）；中文/其他 → 中英双语模型
  - `modelForInitialize(language)`：初始化实选——理想模型已下载则用，
    否则退回任意已下载模型，均无返回 null
  - 修正 `vits-ljs` 逐文件下载 URL（HF 仓库实际只有 int8 量化版
    `vits-ljs.int8.onnx`，原 `model.onnx` 路径 404；GitHub tarball
    主路径已验证可达）
- `TtsHelper`：`initializeEmbedded/initializeEmbeddedForced` 接收语言并
  走模型路由；新增 `switchEmbeddedModelIfNeeded(language)`——换书后
  已加载模型与新书语言不匹配且对应模型已下载时重新初始化，防跨书串声
- `ReaderViewModel`：单段朗读/自动朗读/速读/RSVP 四条启动路径播放前
  复查模型匹配；引导弹窗按本书语言推荐模型（英文书给"下载纯英文语音"
  入口）；下载/重试/失败文案全部按语言路由的模型取信息；
  英文书回退到双语声时一次性 toast 引导去设置下载纯英文模型

**验证**：编译通过、detekt 0 issue、73 单测全过。实机听感需用户在
设备上确认（英文书首次播放会提示/引导下载 ~109MB 纯英文模型；
已下载则自动切换，无需操作）。

**遗留**（记入延期）：语调表现力的上限受模型制约——进一步改善需引入
Piper 系英文模型（更自然的韵律，需 espeak-ng-data 打包，独立任务）；
设置页缺少模型选择列表（当前仅当前模型的下载/删除，自动路由已覆盖
主场景）。
